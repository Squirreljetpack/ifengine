...
use quote::quote;
use syn::{
    Arm, Block, Error, Expr, ExprClosure, ItemFn, Result, Token, Type, braced,
...
struct DChoicesInput {
    ...
    pub choices_expr: Expr,
    pub match_type: Option<Type>,
    pub arms: Vec<Arm>,
}

impl Parse for DChoicesInput {
    fn parse(input: ParseStream) -> Result<Self> {
        // Parse first expr (key or choices)
        let first_expr: Expr = input.parse()?;

        // Determine if first_expr is key or choices
        let maybe_key_and_choices = {
            let fork = input.fork();
            fork.parse::<Token![,]>()?;
            let next_token = fork.lookahead1();
            if next_token.peek(Token![match]) {
                (MaybeKey::Auto, first_expr)
            } else {
                match first_expr {
                    Expr::Paren(x) => {
                        input.parse::<Token![,]>()?;
                        let choices_expr: Expr = input.parse()?;
                        (MaybeKey::Key(*x.expr), choices_expr)
                    }
                    _ => {
                        return Err(syn::Error::new_spanned(
                            first_expr,
                            "If providing a key, it must be parenthesized",
                        ));
                    }
                }
            }
        ...
        let (maybe_key, choices_expr) = maybe_key_and_choices;

        // Expect comma and `match`
        input.parse::<Token![,]>()?;
        input.parse::<Token![match]>()?;

        // Optional type
        let user_type = if input.peek(token::Brace) {
            None
        } else {
            Some(input.parse()?)
        };
...
/// dchoices!(
///     choices,    // Choice list
///     match DChoices {  // match enum type
///         DChoices::A => { dbg!("A handled"); }
///         DChoices::B => { dbg!("B handled"); }
///         DChoices::C => { todo!("Handle C"); }
///     }
/// );
/// ```
...
pub fn dchoices(input: TokenStream) -> TokenStream {
    ...
    let match_tokens = if let Some(ty) = match_type {
        quote! {
            match unsafe { std::mem::transmute::<u8, #ty>(__ifengine_chosen_discriminant) } {
                #(#arms),*
            }
        }
    } else {
        quote! {
            match __ifengine_chosen_discriminant {
                #(#arms),*,
                _ => {}
            }
        }
...
#[proc_macro]
pub fn dynamic_paragraph(input: TokenStream) -> TokenStream {
...
#[proc_macro]
pub fn dynamic_paragraph_masked(input: TokenStream) -> TokenStream {
    let KeyExprInput { maybe_key, expr } = syn::parse_macro_input!(input as KeyExprInput);
    ...
    let expanded = quote! {{
        ...
        __ifengine_page_state.get_mask::<64>(#key)
    }};
...
pub fn read_key_mask(input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        __ifengine_page_state.get_mask(#expr)
    };
