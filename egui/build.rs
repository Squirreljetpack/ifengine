    ...
    for entry in fs::read_dir(&to_absolute(FONTS_PATH)).unwrap() {
        ...
        if path.is_dir()
            && !path
                .file_name()
                .and_then(|f| f.to_str())
                .and_then(|f| Some(f.starts_with('_')))
                .unwrap_or(true)
        {
...
fn add_colors_from_theme(output: &mut String, theme_file: &Path, variant: &str) {
    ...
    let mappings: Vec<(&str, Vec<&str>, Vec<&str>)> = vec![
        ...
        (
            ...
            vec!["token.info-token"],
        ),
        (
            "PURPLE",
            vec!["terminal.ansiBrightMagenta", "terminal.ansiMagenta"],
            vec!["token.debug-token"],
        ),
        (
            "BLACK",
            vec!["terminal.ansiBlack", "editor.background"],
            vec![],
        ),
        (
            "WHITE",
            vec!["terminal.ansiWhite", "editor.foreground"],
            vec![],
        ),
        (
            "GRAY",
        ...
        (
            "PRIMARY",
            vec![],
            vec!["variable", "meta.function-call.arguments"],
        ),
    ...
    map_output
        .push_str("    pub static COLORS_MAP: phf::Map<&'static str, Color32> = phf_map! {\n");
